...
This layer is deliberately small and deterministic. It does not replace the v6
source archaeology plane, fetch upstream code, or confer authority from a link.
It selects bounded reference packets, prefers permissive sources when capability
is comparable, and preserves license/research obligations through completion.
"""
...
def _license_status(candidate: dict[str, Any]) -> str:
    ...
    return str(license_data.get("status", "unverified")) if isinstance(license_data, dict) else "unverified"
...
def rank_reference_candidates(candidates: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    """Rank candidates while making permissive reuse the default tie-breaker.

    Capability remains primary. A restrictive source can win only when its
    unique-requirement fit is materially stronger; a small quality/style edge
    cannot defeat an otherwise suitable permissive implementation.
    """
...
def _source_map(network: dict[str, Any]) -> dict[str, dict[str, Any]]:
    return {
        ...
        for source in network.get("sources", [])
...
def resolve_reference_pack(
    ...
    *,
    ...
    """Resolve a bounded packet and retain its lifecycle obligations."""
    if max_sources < 1:
...
def validate_external_ui_network(
    ...
    for index, source in enumerate(sources):
        ...
        if status == "green" and license_data.get("id") not in PERMISSIVE_LICENSES:
    ...
    for index, pack in enumerate(pack_items):
        ...
        preferred = [by_id[source_id] for source_id in pack.get("preferred_sources", []) if source_id in by_id]
        has_green = any(
            _license_status(source) == "green" and source.get("license", {}).get("id") in PERMISSIVE_LICENSES
