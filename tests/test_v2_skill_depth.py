...
class V2SkillDepthTests(unittest.TestCase):
    def test_manifest_ownership_is_unique_and_specific(self):
        ...
        for item in MANIFEST["skills"]:
            self.assertTrue(item["ownership"].strip(), item["name"])

    def test_every_v2_skill_has_behavioral_contract_structure(self):
        ...
        for item in MANIFEST["skills"]:
            ...
            self.assertIn("Evidence", text, f"{item['name']} lacks evidence semantics")
